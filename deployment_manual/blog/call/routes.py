...
# Удаленные Oracle-модели: AgencyPhone, CallInfo - используем прямые SQL запросы
...
def get_agency_id_by_ani(ani) -> Union[Any, Literal["0"]]:
    ...
    try:
        with SessionSalesSchema() as session:
            # Прямой SQL запрос к Oracle таблице T_AGENCY_PHONE
            query = text("""
                SELECT AGENCY_ID
                FROM T_AGENCY_PHONE
                WHERE AGENCY_PHONE = :ani
                ORDER BY AGENCY_ID DESC
                FETCH FIRST 1 ROWS ONLY
            """)
            result = session.execute(query, {"ani": ani}).fetchone()
...
def handle_endcall():
    try:
        ...
        if pk_record is not None:
            ...
            # Получаем информацию о звонке для отображения на странице через прямой SQL запрос
            try:
                with SessionSalesSchema() as session:
                    query = text("""
                    ...
                    """)
                    result = session.execute(query, {"pk_record": pk_record}).fetchone()

                    if result:
                        call_dict = dict(result._mapping)
                ...
                logger.error(f"Ошибка получения данных звонка: {str(e)}")
                return render_template("endcall.html", call={"phone_number": ani})
...
def get_pk_record():
    try:
        with SessionSalesSchema() as session:
            ...
            query = text("SELECT MAX(CALL_INFO_ID) FROM T_CALL_INFO")
    ...
    try:
        ...
        try:
            with SessionSalesSchema() as session:
                ...
                limit_clause = (
                    "FETCH FIRST :limit ROWS ONLY" if dialect_name == "oracle" else "LIMIT :limit"
                )
...
def get_last_calls_all(limit: int = 5) -> List[Dict[str, Any]]:
    ...
    Используем универсальный SQL, который корректно работает как в Oracle (FETCH FIRST)
    так и в SQLite/PostgreSQL/MySQL (LIMIT)."""

    try:
        with SessionSalesSchema() as session:
            dialect_name = session.bind.dialect.name.lower()

            # Определяем корректную конструкцию ограничения количества строк
            limit_clause = (
                "FETCH FIRST :limit ROWS ONLY" if dialect_name == "oracle" else "LIMIT :limit"
            )
...
def get_max_call_info_id(db_session):
    """Функция больше не используется - заменена на прямые SQL запросы"""
    try:
...
def get_latest_calls():
    try:
        ...
        with SessionSalesSchema() as session:
            ...
            # Определяем корректную конструкцию ограничения количества строк
            limit_clause = (
                "FETCH FIRST 5 ROWS ONLY" if dialect_name == "oracle" else "LIMIT 5"
            )
    ...
    if connection:
        try:
            with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                ...
                # Приводим результат к списку словарей для удобства и корректной типизации
                raw_calls = [dict(r) for r in raw_calls]
