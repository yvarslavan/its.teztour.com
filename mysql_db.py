...
def get_database_config():
    ...
    if mysql_host and ':' in mysql_host:
        mysql_host, mysql_port = mysql_host.split(':', 1)
    ...
    # Обработка хоста качества
    mysql_quality_host = os.getenv('MYSQL_QUALITY_HOST', '127.0.0.1')
    mysql_quality_port = os.getenv('MYSQL_QUALITY_PORT', '3306')
    if mysql_quality_host and ':' in mysql_quality_host:
        mysql_quality_host, mysql_quality_port = mysql_quality_host.split(':', 1)

    mysql_quality_config = {
        'host': mysql_quality_host,
        'port': mysql_quality_port,
    ...
    return {
        'mysql': mysql_config,
        'mysql_quality': mysql_quality_config
    }
...
QUALITY_DATABASE_URL = (
    ...
    f"{db_config['mysql_quality']['port']}/{db_config['mysql_quality']['database']}"
)
