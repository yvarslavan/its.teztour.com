...
class SecureConfig:
    ...
    def load_from_env(self):
        ...
        if self.mysql_host and ":" in self.mysql_host:
            self.mysql_host, self.mysql_port = self.mysql_host.split(":", 1)
        ...
        if self.mysql_quality_host and ":" in self.mysql_quality_host:
            self.mysql_quality_host, self.mysql_quality_port = (
                self.mysql_quality_host.split(":", 1)
            )
        ...
        self.redmine_quality_password_admin = os.getenv(
            "REDMINE_QUALITY_PASSWORD_ADMIN", ""
        )
        self.redmine_quality_anonymous_user_id = os.getenv(
