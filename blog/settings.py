...
class Config:
    ...
    if _oracle_sales:
        ...
        logger.warning(
            "ВНИМАНИЕ: SQLALCHEMY_SALES_SCHEMA_URI_ORACLE_SALES не установлена!"
        )
        # Фиктивное соединение для безопасного импорта (не будет использоваться)
    ...
    RECOVERY_PASSWORD_URL = os.environ.get("RECOVERY_PASSWORD_URL", "https://mail.tez-tour.com/RecoveryPassword.wssp")
    ...
    @property
    def DB_REDMINE_PORT(self):
        return self.get_db_redmine_config()["port"]
    ...
    try:
        ...
        VAPID_CLAIMS = {"sub": "mailto:y.varslavan@tez-tour.com"}
