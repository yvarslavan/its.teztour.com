...
class KanbanTooltips {
    ...
    /**
     * Отключает всплывающие подсказки
     */
    disable() {
        console.log('[KanbanTooltips] 🚫 Отключение всплывающих подсказок');
        this.isEnabled = false;
        this.cleanup();
    }

    /**
     * Включает всплывающие подсказки
     */
    enable() {
        console.log('[KanbanTooltips] ✅ Включение всплывающих подсказок');
        this.isEnabled = true;

        // Сбрасываем глобальный флаг отключения
        if (window.kanbanTooltipsDisabled) {
            window.kanbanTooltipsDisabled = false;
            console.log('[KanbanTooltips] 🔄 Сброшен глобальный флаг отключения');
        }
    ...
    init() {
        if (!this.isEnabled) {
            console.log('[KanbanTooltips] 🚫 Подсказки отключены, пропускаем инициализацию');
            return;
        }
        ...
        if (!showKanbanTips) {
            console.log('[KanbanTooltips] 🚫 Подсказки отключены пользователем, пропускаем инициализацию');
            return;
        }

        // Проверяем глобальный флаг отключения
        if (window.kanbanTooltipsDisabled) {
            console.log('[KanbanTooltips] 🚫 Подсказки глобально отключены, пропускаем инициализацию');
            return;
        }
...
// Глобальная функция для отключения всех подсказок навсегда
window.disableAllKanbanTooltips = function() {
    console.log('[Global] 🚫 Отключение всех Kanban подсказок навсегда');

    if (typeof window.KanbanTooltips !== 'undefined') {
        // Создаем экземпляр и отключаем
        const tooltips = new window.KanbanTooltips();
        tooltips.disable();

        // Также скрываем все существующие подсказки
        tooltips.forceHideAllTooltips();

        console.log('[Global] ✅ Все Kanban подсказки отключены навсегда');
    } else {
        // Альтернативный способ - удаляем все подсказки и блокируем их создание
        window.hideAllKanbanTooltips();

        // Устанавливаем глобальный флаг отключения
        window.kanbanTooltipsDisabled = true;

        console.log('[Global] ✅ Все Kanban подсказки отключены (альтернативный способ)');
    }
};

// Глобальная функция для включения всех подсказок обратно
window.enableAllKanbanTooltips = function() {
    console.log('[Global] ✅ Включение всех Kanban подсказок обратно');

    if (typeof window.KanbanTooltips !== 'undefined') {
        // Создаем экземпляр и включаем
        const tooltips = new window.KanbanTooltips();
        tooltips.enable();

        console.log('[Global] ✅ Все Kanban подсказки включены обратно');
    } else {
        // Альтернативный способ - сбрасываем глобальный флаг
        window.kanbanTooltipsDisabled = false;

        console.log('[Global] ✅ Глобальный флаг отключения сброшен');
    }
};
