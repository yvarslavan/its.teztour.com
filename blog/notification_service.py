...
try:
    ...
    DB_REDMINE_HOST, DB_REDMINE_DB, DB_REDMINE_USER, DB_REDMINE_PASSWORD = (
        None,
        None,
        None,
        None,
...
class NotificationDeduplicator:
    ...
    def is_duplicate(
        self, notification: NotificationData, request_id: Optional[uuid.UUID] = None
    ) -> bool:
        """Проверка на дублирование"""
        # Используем request_id в логах, если он передан
        log_prefix = (
            f"[REQ_ID:{request_id}][DEDUPLICATOR]" if request_id else "[DEDUPLICATOR]"
        )
        ...
        with self._lock:
            ...
            if hash_key in self._cache:
                logger.info(
                    f"{log_prefix} Проверка хеша: {hash_key}. Результат: ДУБЛИКАТ (Найден в кеше). Время в кеше: {self._cache[hash_key]}"
                )
                return True
            ...
            logger.info(
                f"{log_prefix} Проверка хеша: {hash_key}. Результат: НОВЫЙ. Добавлен в кеш с временем {now}"
            )
            return False
...
class NotificationService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"[NotificationService] push_service INITIALIZED: {type(self.push_service)}"
        )
    ...
    def process_notifications(self, user_email: str, user_id: int) -> int:
        ...
        return self._sync_requester_queue_notifications(
            user_id=user_id,
    ...
    @staticmethod
    def _normalize_recipient_email(user_email: Optional[str]) -> Optional[str]:
        """Нормализует email получателя для поиска в очередях уведомлений."""
        if not user_email:
            return None
    ...
    def _sync_requester_queue_notifications(
        self,
        user_id: int,
        user_email: Optional[str] = None,
        request_id: Optional[uuid.UUID] = None,
    ) -> int:
        """Синхронизирует очереди статусных и коммент-уведомлений для заявителя."""
        request_id = request_id or uuid.uuid4()
        recipient_email = self._normalize_recipient_email(user_email)

        if recipient_email is None:
            user = User.query.get(user_id)
            recipient_email = self._normalize_recipient_email(
                user.email if user else None
            )

        logger.info(
            f"[REQ_ID:{request_id}] Начало синхронизации requester-очереди для user_id={user_id}, recipient_email={recipient_email}"
        )

        if recipient_email is None:
            logger.info(
                f"[REQ_ID:{request_id}] Синхронизация requester-очереди пропущена: у пользователя {user_id} нет email."
            )
            return 0
        ...
        try:
            ...
            if not connection:
                logger.error(
                    f"[REQ_ID:{request_id}] Не удалось подключиться к MySQL для user_id={user_id}, recipient_email={recipient_email}"
                )
                return 0

            logger.info(
                f"[REQ_ID:{request_id}] Успешно подключились к MySQL для user_id={user_id}, recipient_email={recipient_email}"
            )
            ...
            try:
                status_count = self._process_status_notifications(
                    connection, recipient_email, user_id, request_id
                )
                ...
                logger.info(
                    f"[REQ_ID:{request_id}] Синхронизация requester-очереди завершена: user_id={user_id}, recipient_email={recipient_email}, processed={total_processed}"
                )
                return total_processed
            finally:
                connection.close()
                logger.info(
                    f"[REQ_ID:{request_id}] Соединение с MySQL закрыто для user_id={user_id}, recipient_email={recipient_email}"
                )

        except Exception as e:
            logger.error(
                f"[REQ_ID:{request_id}] Ошибка при синхронизации requester-очереди для user_id={user_id}, recipient_email={recipient_email}: {e}",
                exc_info=True,
            )
            return 0
    ...
    def _fetch_and_save_redmine_notifications(self, user_id: int):
        ...
        try:
            ...
            if not source_connection:
                logger.error(
                    f"Не удалось подключиться к MySQL для Redmine уведомлений (пользователь {user_id})"
                )
                return
            ...
            with source_connection.cursor(pymysql.cursors.DictCursor) as cursor:
                ...
                for row in notifications_from_source:
                    try:
                        ...
                        exists = (
                            db.session.query(RedmineNotification)
                            .filter_by(
                                user_id=user_id, source_notification_id=row["id"]
                            )
                            .first()
                        )
                        ...
                        if not exists:
                            ...
                            is_group = (
                                bool(group_name_value and group_name_value.strip())
                                if group_name_value
                                else False
                            )
                            ...
                            new_notification = RedmineNotification(
                                ...
                                easy_email_to=row.get(
                                    "author_email"
                                ),  # Используем author_email из MySQL
                        ...
                        logger.error(
                            f"Ошибка при обработке Redmine уведомления (source_id: {row.get('id', 'N/A')}): {e}"
                        )
                        db.session.rollback()
    ...
    def _process_status_notifications(
        self, connection, user_email: str, user_id: int, request_id: uuid.UUID
    ) -> int:
        """Обработка уведомлений об изменении статуса"""
        logger.info(
            f"[REQ_ID:{request_id}] _process_status_notifications для user_id={user_id}, recipient_email={user_email}"
        )
        cursor = None
        try:
            ...
            if recipient_email is None:
                logger.info(
                    f"[REQ_ID:{request_id}] Пропуск обработки статусов: recipient_email пуст для user_id={user_id}"
                )
                return 0
            ...
            logger.info(
                f"[REQ_ID:{request_id}] MySQL Query (status): WHERE Author = ? для recipient_email={recipient_email}"
            )
            cursor.execute(query, (recipient_email,))
            rows = cursor.fetchall()
            source_ids = [row["ID"] for row in rows]
            logger.info(
                f"[REQ_ID:{request_id}] Получено {len(rows)} строк из u_its_update_status для recipient_email={recipient_email}. IDs: {source_ids}"
            )
            ...
            for row in rows:
                logger.info(
                    f"[REQ_ID:{request_id}] Queue pickup status: source_id={row['ID']}, issue_id={row['IssueID']}, recipient_email={recipient_email}, row_author={row.get('Author')}"
                )
                ...
                if not self.deduplicator.is_duplicate(notification_data, request_id):
                    logger.info(
                        f"[REQ_ID:{request_id}] Статус-уведомление source_id={row['ID']} не дубликат, добавляем к сохранению для recipient_email={recipient_email}."
                    )
                    notifications_to_save.append(notification_data)
                    processed_count += 1
                else:
                    logger.info(
                        f"[REQ_ID:{request_id}] Статус-уведомление source_id={row['ID']} определено как дубликат Deduplicator-ом для recipient_email={recipient_email}."
                    )

            if notifications_to_save:
                logger.info(
                    f"[REQ_ID:{request_id}] Планируется сохранить {len(notifications_to_save)} статус-уведомлений для recipient_email={recipient_email}."
                )

                successfully_saved_ids = self._save_status_notifications(
                    notifications_to_save, request_id
                )

                if successfully_saved_ids:
                    self._delete_processed_status_notifications(
                        connection,
                    ...
                    logger.info(
                        f"[REQ_ID:{request_id}] Нет успешно сохраненных статус-уведомлений для удаления из MySQL для recipient_email={recipient_email}."
                    )
            else:
                logger.info(
                    f"[REQ_ID:{request_id}] Нет новых статус-уведомлений для сохранения для recipient_email={recipient_email}."
                )
            ...
            logger.error(
                f"[REQ_ID:{request_id}] Ошибка при обработке уведомлений о статусах: {e}",
                exc_info=True,
            )
            return 0
        finally:
    ...
    def _process_comment_notifications(
        self, connection, user_email: str, user_id: int, request_id: uuid.UUID
    ) -> int:
        """Обработка уведомлений о комментариях"""
        logger.info(
            f"[REQ_ID:{request_id}] _process_comment_notifications для user_id={user_id}, recipient_email={user_email}"
        )
        cursor = None
        try:
            cursor = connection.cursor(pymysql.cursors.DictCursor)
            ...
            if recipient_email is None:
                logger.info(
                    f"[REQ_ID:{request_id}] Пропуск обработки комментариев: recipient_email пуст для user_id={user_id}"
                )
                return 0
            ...
            logger.info(
                f"[REQ_ID:{request_id}] MySQL Query (comment): WHERE Author = ? для recipient_email={recipient_email}"
            )
            cursor.execute(query, (recipient_email,))
            rows = cursor.fetchall()
            source_ids = [row["ID"] for row in rows]
            logger.info(
                f"[REQ_ID:{request_id}] Получено {len(rows)} строк из u_its_add_notes для recipient_email={recipient_email}. IDs: {source_ids}"
            )
            ...
            for row in rows:
                logger.info(
                    f"[REQ_ID:{request_id}] Queue pickup comment: source_id={row['ID']}, issue_id={row['issue_id']}, recipient_email={recipient_email}, row_author={row.get('Author')}"
                )
                ...
                author_name = (
                    author_result["author_name"] if author_result else row["Author"]
                )
                ...
                if not self.deduplicator.is_duplicate(notification_data, request_id):
                    logger.info(
                        f"[REQ_ID:{request_id}] Коммент-уведомление source_id={row['ID']} не дубликат, добавляем к сохранению для recipient_email={recipient_email}."
                    )
                    notifications_to_save.append(notification_data)
                    processed_count += 1
                else:
                    logger.info(
                        f"[REQ_ID:{request_id}] Коммент-уведомление source_id={row['ID']} определено как дубликат Deduplicator-ом для recipient_email={recipient_email}."
                    )

            if notifications_to_save:
                logger.info(
                    f"[REQ_ID:{request_id}] Планируется сохранить {len(notifications_to_save)} коммент-уведомлений для recipient_email={recipient_email}."
                )

                successfully_saved_ids = self._save_comment_notifications(
                    notifications_to_save, request_id
                )

                if successfully_saved_ids:
                    self._delete_processed_comment_notifications(
                        connection,
                        successfully_saved_ids,
                        request_id,
                        recipient_email,
                    ...
                    logger.info(
                        f"[REQ_ID:{request_id}] Нет успешно сохраненных коммент-уведомлений для удаления из MySQL для recipient_email={recipient_email}."
                    )
            else:
                logger.info(
                    f"[REQ_ID:{request_id}] Нет новых коммент-уведомлений для сохранения для recipient_email={recipient_email}."
                )
            ...
            logger.error(
                f"[REQ_ID:{request_id}] Ошибка при обработке уведомлений о комментариях: {e}",
                exc_info=True,
            )
            return 0
        finally:
    ...
    def _save_status_notifications(
        self, notifications: List[NotificationData], request_id: uuid.UUID
    ) -> List[int]:
        """Сохранение уведомлений об изменении статуса в базу данных. Возвращает список успешно сохраненных MySQL ID."""
        logger.info(
            f"[REQ_ID:{request_id}] _save_status_notifications: {len(notifications)} уведомлений"
        )
        saved_count = 0
        ...
        try:
            for notification_data in notifications:
                try:
                    recipient_email = notification_data.data.get("recipient_email")
                    ...
                    if existing_notification:
                        logger.info(
                            f"[REQ_ID:{request_id}] Статус-уведомление уже существует (DB ID: {existing_notification.id}, source_id={notification_data.source_id}, issue_id={notification_data.issue_id}, recipient_email={recipient_email}). Пропуск."
                        )
                        continue
                    ...
                    logger.info(
                        f"[REQ_ID:{request_id}] Local save status prepared: source_id={notification_data.source_id}, db_id={new_notification_db.id}, issue_id={notification_data.issue_id}, recipient_email={recipient_email}"
                    )
                    ...
                    db.session.rollback()
                    logger.error(
                        f"[REQ_ID:{request_id}] Ошибка SQLAlchemy при сохранении статус-уведомления: {e}",
                        exc_info=True,
                    )
                except Exception as e:
                    db.session.rollback()
                    logger.error(
                        f"[REQ_ID:{request_id}] Непредвиденная ошибка при подготовке статус-уведомления: {e}",
                        exc_info=True,
                    )
                    continue
            ...
            if saved_count > 0:
                db.session.commit()
                logger.info(
                    f"[REQ_ID:{request_id}] Успешно сохранено {saved_count} новых статус-уведомлений."
                )
            else:
                logger.info(
                    f"[REQ_ID:{request_id}] Нет новых уникальных статус-уведомлений для сохранения."
                )

        except RuntimeError as e:
            if "Working outside of application context" in str(e):
                logger.error(
                    f"[REQ_ID:{request_id}] КРИТИЧЕСКАЯ ОШИБКА КОНТЕКСТА в _save_status_notifications: {e}",
                    exc_info=True,
                )
            else:
                logger.error(
                    f"[REQ_ID:{request_id}] Непредвиденная RuntimeError в _save_status_notifications: {e}",
                    exc_info=True,
                )
        except Exception as e:
            logger.error(
                f"[REQ_ID:{request_id}] Общая ошибка в _save_status_notifications: {e}",
                exc_info=True,
            )
            try:
                db.session.rollback()
        ...
        if newly_created_notifications_for_push:
            logger.info(
                f"[REQ_ID:{request_id}] Отправка {len(newly_created_notifications_for_push)} PUSH о статусах."
            )
            for notification_data_to_push in newly_created_notifications_for_push:
                try:
                    ...
                    logger.error(
                        f"[REQ_ID:{request_id}] Ошибка отправки PUSH о статусе для source_id={notification_data_to_push.source_id}, issue_id={notification_data_to_push.issue_id}, recipient_email={notification_data_to_push.data.get('recipient_email')}: {push_error}",
                        exc_info=True,
                    )
        else:
    ...
    def _save_comment_notifications(
        self, notifications: List[NotificationData], request_id: uuid.UUID
    ) -> List[int]:
        """Сохранение уведомлений о комментариях в базу данных. Возвращает список успешно сохраненных MySQL ID."""
        logger.info(
            f"[REQ_ID:{request_id}] _save_comment_notifications: {len(notifications)} уведомлений"
        )
        saved_count = 0
        newly_created_notifications_for_push = []
        successfully_saved_mysql_ids = []
        ...
        if not notifications:
            logger.info(
                f"[REQ_ID:{request_id}] Нет коммент-уведомлений для сохранения."
            )
            return []
        ...
        try:
            for notification_data in notifications:
                try:
                    ...
                    if existing_notification:
                        logger.info(
                            f"[REQ_ID:{request_id}] Коммент-уведомление уже существует (DB ID: {existing_notification.id}, source_id={notification_data.source_id}, issue_id={notification_data.issue_id}, recipient_email={recipient_email}). Пропуск."
                        )
                        continue
                    ...
                    logger.info(
                        f"[REQ_ID:{request_id}] Local save comment prepared: source_id={notification_data.source_id}, db_id={new_notification_db.id}, issue_id={notification_data.issue_id}, recipient_email={recipient_email}"
                    )
                    newly_created_notifications_for_push.append(notification_data)
                    saved_count += 1
                    ...
                    db.session.rollback()
                    logger.error(
                        f"[REQ_ID:{request_id}] Ошибка SQLAlchemy при сохранении коммент-уведомления: {e}",
                        exc_info=True,
                    )
                except Exception as e:
                    db.session.rollback()
                    logger.error(
                        f"[REQ_ID:{request_id}] Непредвиденная ошибка при подготовке коммент-уведомления: {e}",
                        exc_info=True,
                    )
                    continue
            ...
            if saved_count > 0:
                db.session.commit()
                logger.info(
                    f"[REQ_ID:{request_id}] Успешно сохранено {saved_count} новых коммент-уведомлений."
                )
            else:
                logger.info(
                    f"[REQ_ID:{request_id}] Нет новых уникальных коммент-уведомлений для сохранения."
                )

        except RuntimeError as e:
            if "Working outside of application context" in str(e):
                logger.error(
                    f"[REQ_ID:{request_id}] КРИТИЧЕСКАЯ ОШИБКА КОНТЕКСТА в _save_comment_notifications: {e}",
                    exc_info=True,
                )
            else:
                logger.error(
                    f"[REQ_ID:{request_id}] Непредвиденная RuntimeError в _save_comment_notifications: {e}",
                    exc_info=True,
                )
        except Exception as e:
            logger.error(
                f"[REQ_ID:{request_id}] Общая ошибка в _save_comment_notifications: {e}",
                exc_info=True,
            )
            try:
                db.session.rollback()
        ...
        if newly_created_notifications_for_push:
            logger.info(
                f"[REQ_ID:{request_id}] Отправка {len(newly_created_notifications_for_push)} PUSH о комментариях."
            )
            for notification_data_to_push in newly_created_notifications_for_push:
                try:
                    self.push_service.send_push_notification(notification_data_to_push)
                    ...
                    logger.error(
                        f"[REQ_ID:{request_id}] Ошибка отправки PUSH о комментарии для source_id={notification_data_to_push.source_id}, issue_id={notification_data_to_push.issue_id}, recipient_email={notification_data_to_push.data.get('recipient_email')}: {push_error}",
                        exc_info=True,
                    )
        else:
    ...
    def _delete_processed_status_notifications(
        self,
        connection,
        ...
        logger.info(
            f"[REQ_ID:{request_id}] Queue delete status: recipient_email={recipient_email}, IDs={ids_to_delete}"
        )
        ...
        try:
            ...
            connection.commit()
            logger.info(
                f"[REQ_ID:{request_id}] Удалено {cursor.rowcount} записей о статусах из MySQL для recipient_email={recipient_email} (IDs: {ids_to_delete})"
            )
            cursor.close()
        except Exception as e:
            logger.error(
                f"[REQ_ID:{request_id}] Ошибка при удалении обработанных статус-уведомлений для recipient_email={recipient_email} (IDs: {ids_to_delete}): {e}",
                exc_info=True,
            )
    ...
    def _delete_processed_comment_notifications(
        self,
        connection,
        ids_to_delete: List[int],
        request_id: uuid.UUID,
        ...
        logger.info(
            f"[REQ_ID:{request_id}] Queue delete comment: recipient_email={recipient_email}, IDs={ids_to_delete}"
        )
        if not ids_to_delete:
            return
        try:
            ...
            connection.commit()
            logger.info(
                f"[REQ_ID:{request_id}] Удалено {cursor.rowcount} записей о комментариях из MySQL для recipient_email={recipient_email} (IDs: {ids_to_delete})"
            )
            cursor.close()
        except Exception as e:
            logger.error(
                f"[REQ_ID:{request_id}] Ошибка при удалении обработанных коммент-уведомлений для recipient_email={recipient_email} (IDs: {ids_to_delete}): {e}",
                exc_info=True,
            )
    ...
    def get_user_notifications(self, user_id: int) -> Dict:
        ...
        try:
            self._sync_requester_queue_notifications(user_id=user_id)
            ...
            all_status_notifications = (
                Notifications.query.filter_by(user_id=user_id)
                .order_by(Notifications.date_created.desc())
                .all()
            )

            status_notifications = [
                n
                for n in all_status_notifications
                if n.is_read is None or n.is_read is False
            ]

            all_comment_notifications = (
                NotificationsAddNotes.query.filter_by(user_id=user_id)
                .order_by(NotificationsAddNotes.date_created.desc())
                .all()
            )

            comment_notifications = [
                n
                for n in all_comment_notifications
                if n.is_read is None or n.is_read is False
            ]
            ...
            total_count = (
                len(status_data) + len(comment_data) + len(redmine_notifications)
            )
            ...
            logger.error(
                f"Ошибка при получении уведомлений пользователя {user_id}: {e}"
            )
            return {
                "status_notifications": [],
                "comment_notifications": [],
            ...
            logger.error(
                f"Общая ошибка при получении уведомлений пользователя {user_id}: {e}"
            )
            return {
                "status_notifications": [],
                "comment_notifications": [],
    ...
    def get_local_redmine_notifications(self, user_id: int) -> List[Dict]:
        ...
        try:
            # ИСПРАВЛЕНО: Сортируем по убыванию даты (новые в начале)
            redmine_notifications = (
                RedmineNotification.query.filter_by(user_id=user_id)
                .order_by(RedmineNotification.created_at.desc())
                .all()
            )
            ...
            logger.error(
                f"Ошибка при получении локальных Redmine уведомлений для user_id={user_id}: {e}",
                exc_info=True,
            )
            return []
    ...
    def get_notifications_for_page(self, user_id: int) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Ошибка при получении уведомлений для страницы user_id={user_id}: {e}"
            )
            return {
                "status_notifications": [],
                "comment_notifications": [],
    ...
    def get_redmine_notifications(self, user_id: int) -> List[Dict]:
        ...
        if not connection:
            logger.error(
                f"Не удалось получить соединение с Redmine DB для пользователя {user_id}"
            )
            return []
        ...
        try:
                ...
                redmine_data.append(
                    {
                        ...
                        "is_group_notification": (
                            bool(
                                item.get("group_name")
                                and str(item.get("group_name")).strip()
                            )
                        ...
                        "group_name": item.get(
                            "group_name"
                        ),  # ИСПРАВЛЕНО: Добавляем само имя группы
            ...
            logger.error(
                f"Ошибка при получении Redmine уведомлений для user_id={user_id}: {e}",
                exc_info=True,
            )
            return []
        finally:
    ...
    def clear_user_notifications(self, user_id: int) -> bool:
        ...
        try:
            ...
            if not connection:
                logger.error(
                    f"Не удалось получить соединение с Redmine DB для очистки уведомлений (user_id={user_id})"
                )
                return False
            ...
            try:
                ...
                logger.error(
                    f"Ошибка при очистке Redmine уведомлений для user_id={user_id}: {e}",
                    exc_info=True,
                )
                if connection:
                    connection.rollback()
            ...
            db.session.rollback()
            logger.error(
                f"Ошибка SQLAlchemy при очистке уведомлений: {e}", exc_info=True
            )
            return False
        except Exception as e:
    ...
    def clear_notifications_for_widget(self, user_id: int) -> bool:
        ...
        try:
            ...
            deleted_comments = NotificationsAddNotes.query.filter_by(
                user_id=user_id
            ).delete()
            db.session.commit()
            logger.info(
                f"[CLEAR_WIDGET] Удалено локальных уведомлений: {deleted_status} статусов, {deleted_comments} комментариев."
            )

            # Шаг 2: Пометка Redmine уведомлений как прочитанных в основной базе Redmine
            redmine_success = self._mark_redmine_notifications_as_read(user_id)
            if not redmine_success:
                logger.error(
                    f"[CLEAR_WIDGET] Не удалось отметить Redmine уведомления как прочитанные для user_id={user_id}"
                )
                return False

            logger.info(
                f"[CLEAR_WIDGET] Успешно завершена очистка для user_id={user_id}"
            )
            return True
    ...
    def mark_redmine_notification_as_read(
        ...
        try:
            ...
            if not user or not user.id_redmine_user:
                logger.warning(
                    f"Пользователь {user_id} не является пользователем Redmine"
                )
                return False
            ...
            if not connection:
                logger.error(
                    f"Не удалось подключиться к Redmine DB для отметки уведомления {notification_id}"
                )
                return False
            ...
            try:
                ...
                if affected_rows > 0:
                    logger.info(
                        f"Отмечено как прочитанное Redmine уведомление id={notification_id} для user_id={user_id}"
                    )
                    return True
                else:
                    logger.warning(
                        f"Redmine уведомление id={notification_id} не найдено для user_id={user_id}"
                    )
                    return False
            ...
            logger.error(
                f"Ошибка при отметке Redmine уведомления id={notification_id} как прочитанного для user_id={user_id}: {e}",
                exc_info=True,
            )
            return False

    def mark_all_notifications_as_read(self, user_id: int) -> bool:
        """Отмечает все уведомления пользователя как прочитанные (для кнопки Очистить в виджете)."""
        try:
            # Отмечаем локальные уведомления как прочитанные (НЕ удаляем!)
            status_notifications = (
                Notifications.query.filter_by(user_id=user_id)
                .filter(
                    or_(Notifications.is_read == None, Notifications.is_read == False)  # type: ignore
                )
                .all()
            )
            ...
            comment_notifications = (
                NotificationsAddNotes.query.filter_by(user_id=user_id)
                .filter(
                    or_(NotificationsAddNotes.is_read == None, NotificationsAddNotes.is_read == False)  # type: ignore
                )
                .all()
            )
            ...
            db.session.commit()
            logger.info(
                f"Отмечены как прочитанные {len(status_notifications)} статусных и {len(comment_notifications)} комментарных уведомлений для user_id={user_id}"
            )

            # Используем существующий метод для отметки Redmine уведомлений
            return self._mark_redmine_notifications_as_read(user_id)

        except Exception as e:
            logger.error(
                f"Ошибка при массовой отметке уведомлений как прочитанных для user_id={user_id}: {e}",
                exc_info=True,
            )
            db.session.rollback()
            return False

    def _mark_redmine_notifications_as_read(self, user_id: int) -> bool:
        """Вспомогательный метод для отметки Redmine уведомлений как прочитанных."""
        user = User.query.get(user_id)
        if not user or not user.id_redmine_user:
            logger.info(
                f"Пользователь {user_id} не является пользователем Redmine, пропускаем MySQL обновление"
            )
            return True
        ...
        if not connection:
            logger.error(
                f"Не удалось подключиться к Redmine DB для массовой отметки уведомлений user_id={user_id}"
            )
            return False
        ...
        try:
            cursor = connection.cursor()
            # Отмечаем все непрочитанные Redmine уведомления как прочитанные
            query = "UPDATE u_redmine_notifications SET is_read = 1 WHERE user_id = %s AND is_read = 0"
            cursor.execute(query, (user.id_redmine_user,))
            affected_rows = cursor.rowcount
            connection.commit()
            ...
            logger.info(
                f"Отмечены как прочитанные {affected_rows} Redmine уведомлений для user_id={user_id}"
            )
            return True
    ...
    def delete_redmine_notification(self, notification_id: int, user_id: int) -> bool:
        ...
        try:
            logger.info(
                f"[DELETE_REDMINE] Попытка удаления локального Redmine уведомления id={notification_id} для user_id={user_id}"
            )
            ...
            if notification:
                logger.info(
                    f"[DELETE_REDMINE] Уведомление id={notification_id} найдено. Удаление..."
                )
                # ИСПРАВЛЕНИЕ: Также помечаем как прочитанное в MySQL, чтобы оно не вернулось при синхронизации
                if notification.source_notification_id:
                    self.mark_redmine_notification_as_read(user_id, notification.source_notification_id)

                db.session.delete(notification)
                db.session.commit()
                logger.info(
                    f"[DELETE_REDMINE] Уведомление id={notification_id} успешно удалено из локальной базы и отмечено в MySQL."
                )
                return True
            else:
                logger.warning(
                    f"[DELETE_REDMINE] Уведомление id={notification_id} не найдено для user_id={user_id}. Проверка без user_id..."
                )
                # Диагностика: попробуем найти уведомление только по ID
                notification_any_user = RedmineNotification.query.filter_by(
                    id=notification_id
                ).first()
                if notification_any_user:
                    logger.warning(
                        f"[DELETE_REDMINE] Уведомление id={notification_id} существует, но для другого user_id: {notification_any_user.user_id}"
                    )
                else:
                    logger.warning(
                        f"[DELETE_REDMINE] Уведомление id={notification_id} вообще не найдено в локальной базе."
                    )
                return False

        except Exception as e:
            logger.error(
                f"[DELETE_REDMINE] Ошибка при удалении локального Redmine уведомления id={notification_id}: {e}",
                exc_info=True,
            )
            db.session.rollback()
            return False
    ...
    def delete_notification(
        self, notification_id: int, notification_type: str, user_id: int
    ) -> bool:
        ...
        try:
            ...
            db.session.commit()
            logger.info(
                f"Удалено уведомление {notification_type}:{notification_id} для пользователя {user_id}"
            )
            return deleted > 0
            ...
            db.session.rollback()
            logger.error(
                f"Ошибка при удалении уведомления {notification_type}:{notification_id}: {e}"
            )
            return False
...
class BrowserPushService:
    ...
    def __init__(self):
        ...
        try:
            ...
            if has_app_context():
                ...
                logger.info(
                    "[BrowserPushService] VAPID ключи загружены при инициализации"
                )
            else:
                logger.info(
                    "[BrowserPushService] Контекст приложения недоступен при инициализации"
                )
        except RuntimeError as e:
            ...
            logger.warning(
                f"[BrowserPushService] Не удалось загрузить VAPID ключи при инициализации: {e}"
            )
    ...
    def _ensure_vapid_config(self):
        ...
        try:
            ...
            try:
                from blog.config.vapid_keys import (
                    VAPID_PUBLIC_KEY,
                    VAPID_PRIVATE_KEY,
                    VAPID_CLAIMS,
                )
                ...
                if VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY:
                    logger.info(
                        f"{log_prefix} VAPID ключи успешно загружены из blog.config.vapid_keys"
                    )
                    ...
                    logger.info(
                        f"{log_prefix} Firebase VAPID ключи установлены из файла"
                    )
                    return True
                else:
                    logger.warning(
                        f"{log_prefix} VAPID ключи пусты в blog.config.vapid_keys"
                    )
            except ImportError as e:
                logger.warning(
                    f"{log_prefix} Не удалось импортировать blog.config.vapid_keys: {e}"
                )
            ...
            if not hasattr(self, "vapid_claims") or not self.vapid_claims:
                ...
                logger.info(
                    f"{log_prefix} VAPID_CLAIMS установлен: {self.vapid_claims}"
                )
            ...
            if config_public_key and config_private_key:
                ...
                if not self.vapid_claims:
                    ...
                    logger.info(
                        f"{log_prefix} VAPID_CLAIMS (повторно для случая ключей из конфига): {self.vapid_claims}"
                    )
                return True
            else:
                logger.error(
                    f"{log_prefix} VAPID ключи НЕ НАЙДЕНЫ в конфигурации Flask!"
                )
                logger.error(
                    f"{log_prefix} КРИТИЧЕСКАЯ ОШИБКА: Необходимо настроить Firebase VAPID ключи!"
                )
                logger.error(f"{log_prefix} Инструкция:")
                logger.error(
                    f"{log_prefix} 1. Перейдите в Firebase Console -> Project Settings -> Cloud Messaging"
                )
                logger.error(
                    f"{log_prefix} 2. Создайте новую пару Web Push certificates"
                )
                logger.error(
                    f"{log_prefix} 3. Обновите blog/config/vapid_keys.py обоими ключами"
                )
            ...
            if "Working outside of application context" in str(e):
                ...
                logger.error(
                    f"{log_prefix} Непредвиденная RuntimeError для VAPID: {e}",
                    exc_info=True,
                )
            return False
        except Exception as e:
    ...
    def send_push_notification(
        self, notification: NotificationData, claims: Optional[Dict] = None
    ):
        ...
        logger.info(
            f"[!!!SEND_PUSH_ENTRY!!!] Вызван send_push_notification для пользователя {notification.user_id}, тип: {notification.notification_type.value}"
        )

        if not WEBPUSH_AVAILABLE:
            logger.warning(
                f"[PUSH_SERVICE] Библиотека pywebpush недоступна. Пропускаем push для пользователя {notification.user_id}"
            )
            return {
                "status": "service_unavailable",
                "message": "pywebpush не доступна",
                "results": [],
            }
        ...
        if not self._ensure_vapid_config():
            logger.error(
                "[PUSH_SERVICE] VAPID конфигурация неполная. Невозможно отправить push-уведомления."
            )
            return {
                "status": "config_error",
                "message": "VAPID конфигурация неполная",
                "results": [],
            }
        logger.info("[PUSH_SERVICE] VAPID конфигурация в порядке.")

        logger.info(
            f"[PUSH_SERVICE] Получение активных подписок для пользователя {notification.user_id}..."
        )
        user_subscriptions = self._get_user_subscriptions(notification.user_id)

        if not user_subscriptions:
            logger.info(
                f"[PUSH_SERVICE] Для пользователя {notification.user_id} не найдено активных подписок."
            )
            return {
        ...
        logger.info(
            f"[PUSH_SERVICE] Найдено {len(user_subscriptions)} активных подписок для пользователя {notification.user_id}."
        )
        ...
        if not icon_url:
            ...
            try:
                ...
                icon_url = url_for(
                    "static", filename="img/push-icon.png", _external=True
                )
            except Exception as e:
                logger.warning(
                    f"[PUSH_SERVICE] Не удалось сгенерировать URL иконки: {e}"
                )
        ...
        if notification.data and "url" in notification.data:
            ...
            try:
                ...
                action_url = url_for(
                    "main.issue", issue_id=notification.issue_id, _external=True
                )
            except Exception:
        ...
        logger.info(
            f"[PUSH_SERVICE] Сформирован основной payload: title='{payload['title']}', message='{payload['message']}', icon='{payload['icon']}'"
        )
        ...
        try:
            ...
            logger.info(
                f"[PUSH_SERVICE] Сформирован payload JSON (первые 200 символов): {payload_json_string[:200]}..."
            )
        except Exception as json_error:
            logger.error(
                f"[PUSH_SERVICE] Ошибка при создании JSON для payload: {json_error}",
                exc_info=True,
            )
            return {
                "status": "payload_error",
                "message": f"Ошибка JSON: {json_error}",
                "results": [],
            }
        ...
        logger.info(
            f"[PUSH_SERVICE] Начало цикла отправки по {total_subscriptions} подпискам для пользователя {notification.user_id}"
        )
        for i, subscription in enumerate(user_subscriptions):
            logger.info(
                f"[PUSH_SERVICE] Итерация {i+1}/{total_subscriptions}. Попытка отправки на подписку ID {subscription.id} (Endpoint: {subscription.endpoint[:50]}...)"
            )
            try:
                self._send_to_subscription(
                    subscription, payload_json_string, claims=claims
                )
                logger.info(
                    f"[PUSH_SEND] Уведомление (предположительно) успешно отправлено на endpoint ID {subscription.id} для пользователя {notification.user_id}"
                )
                results.append(
                    {"subscription_id": subscription.id, "status": "success"}
                )
                success_count += 1
                ...
                logger.error(
                    f"[PUSH_SEND] WebPushException при отправке на endpoint ID {subscription.id} для пользователя {notification.user_id}: {e}",
                    exc_info=True,
                )
                response_status = None
                if hasattr(e, "response") and e.response is not None:
                    logger.error(
                        f"WebPushException response status: {e.response.status_code}"
                    )
                    logger.error(f"WebPushException response text: {e.response.text}")
                    response_status = e.response.status_code
                else:
                    logger.error(
                        "WebPushException не содержит атрибута 'response' или e.response is None."
                    )
                ...
                if response_status in [
                    ...
                    404,
                    410,
                ]:  # 403 (Forbidden - VAPID mismatch), 404 (Not Found), 410 (Gone)
                    logger.warning(
                        f"Подписка ID {subscription.id} вернула статус {response_status}. Деактивируем ее."
                    )
                    try:
                        self._deactivate_subscription(subscription.id)
                    except Exception as de_err:
                        logger.error(
                            f"Ошибка при попытке деактивировать подписку ID {subscription.id}: {de_err}",
                            exc_info=True,
                        )
                ...
                logger.error(
                    f"[PUSH_SEND] Непредвиденная ошибка при отправке на endpoint ID {subscription.id} для пользователя {notification.user_id}: {e}",
                    exc_info=True,
                )
                results.append(
                    {
                ...
                failure_count += 1
        logger.info(
            f"[PUSH_SERVICE] Завершение цикла отправки. Успешно: {success_count}, Неудачно: {failure_count} из {total_subscriptions} для пользователя {notification.user_id}."
        )
        ...
        if success_count > 0 or failure_count > 0:  # Если были какие-либо попытки
            try:
                ...
                logger.error(
                    f"[PUSH_SERVICE] Ошибка сохранения изменений в БД: {db_error}"
                )
                db.session.rollback()

        if failure_count == total_subscriptions and total_subscriptions > 0:
            logger.warning(
                f"[PUSH_SERVICE] Не удалось отправить уведомление ни на одну из {total_subscriptions} подписок для пользователя {notification.user_id}."
            )
            return {
                "status": "all_failed",
                "message": f"Не удалось отправить уведомление ни на одну из {total_subscriptions} подписок.",
                "results": results,
            }
        elif failure_count > 0:
            logger.warning(
                f"[PUSH_SERVICE] Частичный сбой отправки для пользователя {notification.user_id}. Успешно: {success_count}, Неудачно: {failure_count}."
            )
            return {
                "status": "partial_failure",
                "message": f"Успешно: {success_count}, Неудачно: {failure_count}.",
                "results": results,
            }
        elif success_count == total_subscriptions and total_subscriptions > 0:
            logger.info(
                f"[PUSH_SERVICE] Все {success_count} уведомлений успешно отправлены для пользователя {notification.user_id}."
            )
            return {
                "status": "success",
                "message": f"Все {success_count} уведомлений успешно отправлены.",
                "results": results,
            }
        elif total_subscriptions == 0:  # Этот случай уже обработан выше, но для полноты
            logger.info(
                f"[PUSH_SERVICE] Нет активных подписок для отправки пользователю {notification.user_id} (повторная проверка)."
            )
            return {
                "status": "no_subscriptions",
            ...
            logger.error(
                f"[PUSH_SERVICE] Неожиданное состояние счетчиков для пользователя {notification.user_id}: success={success_count}, failure={failure_count}, total={total_subscriptions}"
            )
            return {
                "status": "unknown_error",
                "message": "Неизвестное состояние после отправки",
                "results": results,
            }
    ...
    def _get_user_subscriptions(self, user_id: int) -> List[PushSubscription]:
        ...
        try:
            ...
            logger.error(
                f"Ошибка БД при получении подписок для пользователя {user_id}: {e}",
                exc_info=True,
            )
            return []
    ...
    def _send_to_subscription(
        ...
        payload_json_string: str,
        claims: Optional[Dict] = None,
    ):
        request_id = uuid.uuid4()  # Для трассировки этого конкретного вызова
        logger.critical(
            f"[!!!DEBUG_SEND_SUB_ENTRY!!!] REQ_ID: {request_id} Вызван _send_to_subscription для подписки ID {subscription.id}"
        )

        if not self._ensure_vapid_config():
            logger.error(
                f"[REQ_ID:{request_id}] [PUSH_SEND_SUB] VAPID ключи не настроены. Отправка на подписку ID {subscription.id} отменена."
            )
            # Вместо возврата словаря, который будет проигнорирован, вызываем исключение
            raise WebPushException(
                f"VAPID keys not configured for subscription ID {subscription.id}"
            )
        ...
        if not original_endpoint:
            logger.error(
                f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Пустой endpoint для подписки ID {subscription.id}. Отправка невозможна."
            )
            raise WebPushException(f"Пустой endpoint для подписки ID {subscription.id}")

        # Логируем исходный endpoint
        logger.info(
            f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Исходный endpoint для подписки ID {subscription.id}: '{original_endpoint[:70]}...'"
        )
        ...
        if "fcm.googleapis.com" in endpoint_to_use and not endpoint_to_use.startswith(
            "https://"
        ):
            endpoint_to_use = f"https://{endpoint_to_use}"
            logger.info(
                f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Добавлен протокол https://: '{endpoint_to_use[:70]}...'"
            )
        ...
        if "fcm.googleapis.com/fcm/send/" in endpoint_to_use:
            ...
            logger.info(
                f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Старый формат преобразован в новый: '{endpoint_to_use[:70]}...'"
            )
        ...
        if "fcm.googleapis.com" in endpoint_to_use and "/wp/" not in endpoint_to_use:
            ...
            if len(parts) > 0:
                ...
                if token:
                    endpoint_to_use = f"https://fcm.googleapis.com/wp/{token}"
                    logger.info(
                        f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Добавлен префикс /wp/: '{endpoint_to_use[:70]}...'"
                    )

        # Если endpoint был изменен, логируем это
        if endpoint_to_use != original_endpoint:
            logger.info(
                f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Endpoint адаптирован с '{original_endpoint[:50]}...' на '{endpoint_to_use[:50]}...'"
            )
            ...
            if "fcm.googleapis.com/wp/" in endpoint_to_use:
                ...
                if len(token_parts) > 1 and not token_parts[1]:
                    logger.error(
                        f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Отсутствует токен после /wp/ в адаптированном URL: '{endpoint_to_use}'"
                    )
                    raise WebPushException(
                        f"Некорректный FCM endpoint: отсутствует токен после /wp/"
                    )
        else:
            logger.info(
                f"[REQ_ID:{request_id}] [ADAPT_ENDPOINT_SEND_SUB] Endpoint не требует адаптации: '{endpoint_to_use[:70]}...'"
            )
        ...
        if claims:
            ...
            logger.info(
                f"[REQ_ID:{request_id}] [DEBUG_SEND_SUB] Используются VAPID claims, переданные в функцию: {claims_to_use}"
            )
        else:
            claims_to_use = self.vapid_claims
            logger.info(
                f"[REQ_ID:{request_id}] [DEBUG_SEND_SUB] Используются VAPID claims из экземпляра сервиса: {claims_to_use}"
            )
        ...
        logger.info(
            f"[REQ_ID:{request_id}] [DEBUG_SEND_SUB] Подписка ID {subscription.id}: Endpoint для webpush: {endpoint_to_use}"
        )
        logger.info(
            f"[REQ_ID:{request_id}] [DEBUG_SEND_SUB] Подписка ID {subscription.id}: VAPID Private Key (начало): {self.vapid_private_key[:20] if self.vapid_private_key else 'NOT SET'}"
        )
        logger.info(
            f"[REQ_ID:{request_id}] [DEBUG_SEND_SUB] Подписка ID {subscription.id}: VAPID Claims для отправки: {claims_to_use}"
        )
        ...
        try:
            ...
            logger.info(
                f"[REQ_ID:{request_id}] [SEND_SUCCESS] webpush() вызов успешен для подписки ID {subscription.id}"
            )
            ...
            if endpoint_to_use != original_endpoint:
                try:
                    ...
                    if subscription_db:
                        logger.info(
                            f"[REQ_ID:{request_id}] [ENDPOINT_UPDATE] Обновляем endpoint в БД для подписки ID {subscription.id}"
                        )
                        subscription_db.endpoint = endpoint_to_use
                        db.session.commit()
                        logger.info(
                            f"[REQ_ID:{request_id}] [ENDPOINT_UPDATE] Успешно обновлен endpoint в БД"
                        )
                except Exception as db_err:
                    logger.error(
                        f"[REQ_ID:{request_id}] [ENDPOINT_UPDATE] Ошибка обновления endpoint в БД: {db_err}",
                        exc_info=True,
                    )

        except WebPushException as e:
            logger.error(
                f"WebPushException при отправке на подписку {subscription.id} (endpoint: {subscription.endpoint[:50]}...): {str(e)}",
                exc_info=True,
            )
            response_status = None
            if hasattr(e, "response") and e.response is not None:
            ...
            logger.error(
                f"Ошибка при отправке push-уведомления на подписку {subscription.id} (endpoint: {subscription.endpoint[:50]}...): {str(e)}",
                exc_info=True,
            )
    ...
    def _deactivate_subscription(self, subscription_id: int):
        ...
        try:
            ...
            if sub_to_deactivate:
                ...
                db.session.commit()
                logger.info(
                    f"[DB_UPDATE] Подписка ID {subscription_id} помечена как неактивная."
                )
            else:
                logger.warning(
                    f"[DB_UPDATE] Попытка деактивировать несуществующую подписку ID {subscription_id}."
                )
        except Exception as e:
            logger.error(
                f"[DB_UPDATE] Ошибка при деактивации подписки ID {subscription_id}: {e}",
                exc_info=True,
            )
            db.session.rollback()
    ...
    def send_test_push(self, notification: NotificationData, claims: Dict):
        ...
        logger.info(
            f"[PUSH_SERVICE_TEST] Вызвана send_test_push для пользователя {notification.user_id} с claims: {claims}"
        )

        if not WEBPUSH_AVAILABLE:
            logger.warning(
                f"[PUSH_SERVICE_TEST] pywebpush недоступна, отправка невозможна."
            )
        ...
        if not user_subscriptions:
            logger.info(
                f"[PUSH_SERVICE_TEST] Для пользователя {notification.user_id} не найдено активных подписок."
            )
            return {"status": "no_subscriptions", "message": "Нет активных подписок"}

        # Детальное логирование подписок для отладки
        logger.info(
            f"[PUSH_SERVICE_TEST] Найдено {len(user_subscriptions)} активных подписок для пользователя {notification.user_id}"
        )
        for i, sub in enumerate(user_subscriptions):
            logger.info(
                f"[PUSH_SERVICE_TEST] Подписка {i+1}/{len(user_subscriptions)} - ID: {sub.id}, Endpoint: {sub.endpoint}"
            )
            ...
            if "fcm.googleapis.com" in sub.endpoint:
                if "/fcm/send/" in sub.endpoint:
                    logger.warning(
                        f"[PUSH_SERVICE_TEST] Подписка {sub.id} содержит старый формат FCM URL (/fcm/send/)"
                    )
                elif "/wp/" in sub.endpoint:
                    logger.info(
                        f"[PUSH_SERVICE_TEST] Подписка {sub.id} содержит правильный формат FCM URL (/wp/)"
                    )
                else:
                    logger.warning(
                        f"[PUSH_SERVICE_TEST] Подписка {sub.id} содержит неизвестный формат FCM URL"
                    )
            else:
        ...
        icon_url = notification.data.get(
        ...
        logger.info(
            f"[PUSH_SERVICE_TEST] Попытка отправки на подписку ID {subscription_to_test.id}, Endpoint: {subscription_to_test.endpoint}"
        )
        ...
        if "fcm.googleapis.com" in original_endpoint:
            ...
            if "/fcm/send/" in original_endpoint:
                ...
                logger.info(
                    f"[PUSH_SERVICE_TEST] Адаптирован endpoint старого формата: {adapted_endpoint}"
                )
            # Если отсутствует wp/ - исправляем
                ...
                adapted_endpoint = f"https://fcm.googleapis.com/wp/{token}"
                logger.info(
                    f"[PUSH_SERVICE_TEST] Добавлен префикс /wp/: {adapted_endpoint}"
                )
            # Если нет https:// - добавляем
            if not adapted_endpoint.startswith("https://"):
                adapted_endpoint = f"https://{adapted_endpoint}"
                logger.info(
                    f"[PUSH_SERVICE_TEST] Добавлен протокол https://: {adapted_endpoint}"
                )
        ...
        try:
            ...
            self._send_to_subscription(
                subscription_to_test, payload_json_string, claims=claims
            )
            logger.info(
                f"[PUSH_SERVICE_TEST] Тестовое уведомление успешно отправлено на подписку ID {subscription_to_test.id}"
            )

            # Если отправка прошла успешно и endpoint был изменен, обновляем его в базе данных
            if original_endpoint != adapted_endpoint:
                logger.info(
                    f"[PUSH_SERVICE_TEST] Обновляем endpoint в БД с '{original_endpoint}' на '{adapted_endpoint}'"
                )
                try:
                    subscription_db = PushSubscription.query.get(
                        subscription_to_test.id
                    )
                    if subscription_db:
                        subscription_db.endpoint = adapted_endpoint
                        db.session.commit()
                        logger.info(
                            f"[PUSH_SERVICE_TEST] Endpoint успешно обновлен в БД"
                        )
                except Exception as db_error:
                    logger.error(
                        f"[PUSH_SERVICE_TEST] Ошибка обновления endpoint в БД: {db_error}",
                        exc_info=True,
                    )
            ...
            logger.error(
                f"[PUSH_SERVICE_TEST] Ошибка при отправке тестового уведомления: {e}",
                exc_info=True,
            )
...
def get_notification_service():
    ...
    if _notification_service is None:
        logger.info(
            "[get_notification_service] _notification_service IS NONE, CREATING NEW INSTANCE"
        )
...
class LazyNotificationService:
    ...
    def __getattr__(self, name):
        ...
        if name == "push_service":
            logger.info(
                f"[LazyNotificationService] Accessing push_service, type: {type(service.push_service)}"
            )
...
def debug_notifications_for_user(user_email: str, user_id: int):
    ...
    """
    logger.info(
        f"[DEBUG] Диагностика уведомлений для user_id={user_id}, email={user_email}"
    )

    try:
        normalized_email = NotificationService._normalize_recipient_email(user_email)
        ...
        logger.info(
            f"[DEBUG] Все записи в u_its_update_status: {[(row['ID'], row['IssueID'], row['Author']) for row in all_status_rows]}"
        )
        ...
        logger.info(
            f"[DEBUG] Записи с точным совпадением email '{normalized_email}': {len(exact_match_rows)}"
        )
        ...
        logger.info(
            f"[DEBUG] Записи с похожим email (часть '{email_part}'): {len(similar_rows)}"
        )
        ...
        logger.info(
            f"[DEBUG] Задачи пользователя в issues: {[(row['id'], row['easy_email_to']) for row in user_issues]}"
        )
        ...
        logger.info(
            f"[DEBUG] Email адреса в Redmine: {[(row['user_id'], row['address']) for row in email_addresses]}"
        )
